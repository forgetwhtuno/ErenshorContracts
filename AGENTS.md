...
- Contracts still does not mutate native gameplay state. The current Lunaris build references verified game/UI types only for character identity and fully-in-world readiness. Player-facing UI is retained Unity uGUI and no longer needs Harmony click/camera containment. Do not use UI/readiness access to infer contract progress or alter combat/gameplay.
...
- Do not add Harmony/game hooks merely for retained-UI input handling. In particular, do not add a generic kill/inventory scanner or native reward writer; any future hook needs current assembly evidence and an explicit design reason.
- Do not commit `bin/`, `obj/`, `refs/`, compiled DLLs, game assemblies, or anything under a live Erenshor/Lunaris install path. `.gitignore` already covers the standard cases; don't work around it.
...
- `src/ContractStore.cs` — local sidecar persistence (`plugins/config/ErenshorContracts/`), `.bak`/corrupt recovery.
...
- `src/ContractBoardWindow.cs`, `src/ContractLauncher.cs`, `src/RetainedUiKit.cs` — retained uGUI, including resize/drag/fallback-launcher behavior.
...
- `src/ErenshorContractsPlugin.cs` — native Lunaris plugin entry point, character/readiness context, and retained-UI lifecycle.
- `src/ContractsControlApi.cs`, `src/SuiteUiPolicy.cs` — optional Hub-facing primitive control surface and standalone UI readiness/fallback policy.

## Build / test procedure

- Deterministic core tests: `powershell -ExecutionPolicy Bypass -File .\RUN_TESTS.ps1` (compiles `ContractModels.cs` + `ContractCore.cs` + `tests/ContractCoreTests.cs` standalone via `csc`, no game/Lunaris dependency, safe to run anywhere).
- Full plugin build: `powershell -ExecutionPolicy Bypass -File .\BUILD_AND_INSTALL.ps1` — this locates the current Erenshor/Lunaris install and **installs over the live plugin DLL**. Do not run this as a compile check; prefer a manual `csc` build against local reference DLLs with output redirected elsewhere, or run `RUN_TESTS.ps1` for logic changes.
